...
def select3():         # 학생들 정답 찾기 & 정답과 비교, 채점해서 출력
	...
	os.system('python main.py')           # main.py 실행하면 answerImage에 있는 폴더 모두 실행, txt파일에 정답 저장

	# blank 처리 - len(answerList) - studentAnswerList
	#r = open('C:\\Users\yea\.spyder-py3\\answerSheet\\answerwordLists.txt','rt')
	with codecs.open('/Users/hcy/Desktop/GP/src/answerwordLists.txt','r') as r:
		while(1):
			line = r.readline()
	...
	r.close()
	answerList1 = []
	with codecs.open('/Users/hcy/Desktop/GP/src/answerList.txt','r',encoding='utf-8') as g:
		while(1):
			line = g.readline()
			try:escape=line.index('\r\n')
			except:
